...
17. [图片处理模块](#17-图片处理模块) — 加载、缩放、信息
18. [压缩模块](#18-压缩模块) — ZIP打包、解压、数据压缩
19. [命令行参数模块](#19-命令行参数模块) — 参数解析
20. [配置文件模块](#20-配置文件模块) — TOML/INI读写
21. [HTTP服务端模块](#21-http服务端模块) — 文件服务、API服务
22. [音频模块](#22-音频模块) — 播放、音量控制
23. [Markdown模块](#23-markdown模块) — Markdown转HTML
24. [PDF模块](#24-pdf模块) — PDF文档生成
...
基于 stb_image + stb_image_resize2。

| 函数 | 说明 |
|------|------|
| `图片.加载(路径)` | 加载图片，返回 {宽, 高, 通道, 像素} |
| `图片.信息(路径)` | 获取图片信息（不加载像素），返回 {宽, 高, 通道} |
| `图片.缩放(图片表, 新宽, 新高)` | 缩放图片，返回新图片表 |
| `图片.宽(图片表)` | 获取宽度 |
| `图片.高(图片表)` | 获取高度 |
| `图片.通道(图片表)` | 获取通道数（1=灰度,3=RGB,4=RGBA） |
| `图片.错误()` | 获取最后一次错误信息 |

### 示例

```cp
变量 img = 图片.加载("photo.jpg");
打印("尺寸: " + 转字符串(图片.宽(img)) + "x" + 转字符串(图片.高(img)));
变量 small = 图片.缩放(img, 800, 600);
```
...
基于 miniz。支持 ZIP 打包解压和原始数据压缩。

| 函数 | 说明 |
|------|------|
| `压缩.打包(zip路径, [文件列表])` | 将文件列表打包为 ZIP |
| `压缩.解压(zip路径, 目标目录)` | 解压 ZIP 到目录 |
| `压缩.列表(zip路径)` | 列出 ZIP 包中的文件 |
| `压缩.压缩(数据字符串)` | Deflate 压缩 |
| `压缩.解缩(压缩数据, 原始大小)` | Deflate 解压 |

### 示例

```cp
压缩.打包("backup.zip", ["a.txt", "b.txt", "data/config.toml"]);
变量 files = 压缩.列表("backup.zip");  // ["a.txt","b.txt","config.toml"]
压缩.解压("backup.zip", "output/");
```
...
纯 C++ 实现，支持位置参数、命名参数（--key=value, --flag）、类型转换。

| 函数 | 说明 |
|------|------|
| `参数.创建(描述)` | 创建参数解析器 |
| `参数.加参数(解析器, 名称, 描述, 类型, 必填)` | 添加参数定义，类型="字符串"/"整数"/"浮点"/"布尔" |
| `参数.解析(解析器, 参数数组)` | 解析命令行参数，返回结果表 |
| `参数.命令行()` | 获取程序原始命令行参数 |

### 示例

```cp
变量 p = 参数.创建("文件压缩工具 v1.0");
参数.加参数(p, "--input", "输入文件", "字符串", 真);
参数.加参数(p, "--level", "压缩级别", "整数", 假);
参数.加参数(p, "--verbose", "详细输出", "布尔", 假);
变量 r = 参数.解析(p, ["--input=test.txt", "--level=9"]);
打印(r["--input"]);  // "test.txt"
```
...
支持 TOML（基于 tomlplusplus）和 INI（纯 C++）。

| 函数 | 说明 |
|------|------|
| `配置.读TOML(路径)` | 读取 TOML 文件，返回表 |
| `配置.写TOML(路径, 表)` | 写入 TOML 文件 |
| `配置.读INI(路径)` | 读取 INI 文件，返回 {节: {键: 值}} |
| `配置.写INI(路径, 表)` | 写入 INI 文件 |

### 示例

```cp
// TOML
变量 cfg = 配置.读TOML("config.toml");
打印(cfg["server"]["port"]);  // 8080

// INI
变量 ini = 配置.读INI("settings.ini");
打印(ini["database"]["host"]);  // "localhost"
```
...
基于 Mongoose 嵌入式 Web 服务器。

| 函数 | 说明 |
|------|------|
| `服务.文件(端口, 根目录)` | 启动静态文件服务器 |
| `服务.API(端口)` | 启动简单 API 服务器 |
| `服务.停止(端口)` | 停止服务器 |
...
基于 miniaudio 引擎。支持 WAV/MP3/FLAC 等格式。
...
纯 C++ 实现基础 Markdown 语法。支持标题、粗体、斜体、代码、列表、分隔线。

| 函数 | 说明 |
|------|------|
| `文档.转HTML(md字符串)` | Markdown → HTML |
| `文档.转文本(md字符串)` | 去除 Markdown 标记，提取纯文本 |
...
纯 C++ 实现，生成符合 PDF 1.4 规范的文档。

| 函数 | 说明 |
|------|------|
| `PDF.创建(路径)` | 创建 PDF 文档 |
| `PDF.颜色(句柄, r, g, b)` | 设置当前颜色 (0-255) |
| `PDF.文字(句柄, 文字, x, y, 字号)` | 添加文字 |
| `PDF.直线(句柄, x1, y1, x2, y2)` | 添加直线 |
| `PDF.矩形(句柄, x, y, 宽, 高)` | 添加填充矩形 |
| `PDF.换页(句柄)` | 开始新页面 |
| `PDF.保存(句柄)` | 保存并关闭 PDF |
...
```cp
变量 doc = PDF.创建("report.pdf");
PDF.颜色(doc, 0, 0, 0);
PDF.文字(doc, "CP语言生成报告", 50, 50, 24);
PDF.颜色(doc, 200, 50, 50);
PDF.直线(doc, 50, 80, 550, 80);  // 红色分隔线
PDF.换页(doc);
PDF.文字(doc, "第二页内容", 50, 50, 16);
PDF.保存(doc);
```

---

> 📝 本文档基于 CP 语言 v0.4.0。随着标准库的扩展将持续更新。
