#include "stdlib/stdlib.hpp"

namespace cplang {

// Ordered Map (std::map<Value, Value>) module
// Uses VMOrderedMap which is properly integrated into Value's type system.
// #include'd from stdlib.cpp, already inside namespace cplang

namespace map_ns {

Value mapNew_(std::vector<Value>&) {
    VMOrderedMap* m = VMOrderedMap::create();
    return Value::Ptr(reinterpret_cast<VMObject*>(m));
}

Value mapInsert_(std::vector<Value>& args) {
    if (args.size() < 3 || !args[0].isOrderedMap()) return Value::Bool(false);
    args[0].asOrderedMap()->data[args[1]] = args[2];
    return Value::Bool(true);
}

Value mapFind_(std::vector<Value>& args) {
    if (args.size() < 2 || !args[0].isOrderedMap()) return Value::nil();
    auto it = args[0].asOrderedMap()->data.find(args[1]);
    if (it != args[0].asOrderedMap()->data.end()) return it->second;
    return Value::nil();
}

Value mapContains_(std::vector<Value>& args) {
    if (args.size() < 2 || !args[0].isOrderedMap()) return Value::Bool(false);
    return Value::Bool(args[0].asOrderedMap()->data.find(args[1]) != args[0].asOrderedMap()->data.end());
}

Value mapErase_(std::vector<Value>& args) {
    if (args.size() < 2 || !args[0].isOrderedMap()) return Value::Bool(false);
    return Value::Bool(args[0].asOrderedMap()->data.erase(args[1]) > 0);
}

Value mapSize_(std::vector<Value>& args) {
    if (args.empty() || !args[0].isOrderedMap()) return Value::Int(0);
    return Value::Int(static_cast<Int64>(args[0].asOrderedMap()->data.size()));
}

Value mapIsEmpty_(std::vector<Value>& args) {
    if (args.empty() || !args[0].isOrderedMap()) return Value::Bool(true);
    return Value::Bool(args[0].asOrderedMap()->data.empty());
}

Value mapClear_(std::vector<Value>& args) {
    if (!args.empty() && args[0].isOrderedMap())
        args[0].asOrderedMap()->data.clear();
    return Value::nil();
}

Value mapLowerBound_(std::vector<Value>& args) {
    if (args.size() < 2 || !args[0].isOrderedMap()) return Value::nil();
    auto it = args[0].asOrderedMap()->data.lower_bound(args[1]);
    if (it != args[0].asOrderedMap()->data.end()) return it->first;
    return Value::nil();
}

Value mapUpperBound_(std::vector<Value>& args) {
    if (args.size() < 2 || !args[0].isOrderedMap()) return Value::nil();
    auto it = args[0].asOrderedMap()->data.upper_bound(args[1]);
    if (it != args[0].asOrderedMap()->data.end()) return it->first;
    return Value::nil();
}

Value mapMinKey_(std::vector<Value>& args) {
    if (args.empty() || !args[0].isOrderedMap()) return Value::nil();
    auto& d = args[0].asOrderedMap()->data;
    if (d.empty()) return Value::nil();
    return d.begin()->first;
}

Value mapMaxKey_(std::vector<Value>& args) {
    if (args.empty() || !args[0].isOrderedMap()) return Value::nil();
    auto& d = args[0].asOrderedMap()->data;
    if (d.empty()) return Value::nil();
    return d.rbegin()->first;
}

Value mapKeys_(std::vector<Value>& args) {
    if (args.empty() || !args[0].isOrderedMap()) return Value::nil();
    VMArray* arr = VMArray::create();
    for (auto& kv : args[0].asOrderedMap()->data)
        arr->data.push_back(kv.first);
    return makeArrayVal(arr);
}

Value mapValues_(std::vector<Value>& args) {
    if (args.empty() || !args[0].isOrderedMap()) return Value::nil();
    VMArray* arr = VMArray::create();
    for (auto& kv : args[0].asOrderedMap()->data)
        arr->data.push_back(kv.second);
    return makeArrayVal(arr);
}

Value mapToArray_(std::vector<Value>& args) {
    if (args.empty() || !args[0].isOrderedMap()) return Value::nil();
    VMArray* arr = VMArray::create();
    for (auto& kv : args[0].asOrderedMap()->data) {
        VMTable* pair = VMTable::create();
        pair->set(makeStringVal(VMString::create("key")), kv.first);
        pair->set(makeStringVal(VMString::create("value")), kv.second);
        arr->data.push_back(makeTableVal(pair));
    }
    return makeArrayVal(arr);
}

} // namespace map_ns

void StdLib::registerMap(VM* vm) {
    using namespace map_ns;
    registerFunction(vm, "mapNew",       mapNew_);
    registerFunction(vm, "mapInsert",    mapInsert_);
    registerFunction(vm, "mapFind",      mapFind_);
    registerFunction(vm, "mapContains",  mapContains_);
    registerFunction(vm, "mapErase",     mapErase_);
    registerFunction(vm, "mapSize",      mapSize_);
    registerFunction(vm, "mapIsEmpty",   mapIsEmpty_);
    registerFunction(vm, "mapClear",     mapClear_);
    registerFunction(vm, "mapLowerBound", mapLowerBound_);
    registerFunction(vm, "mapUpperBound", mapUpperBound_);
    registerFunction(vm, "mapMinKey",    mapMinKey_);
    registerFunction(vm, "mapMaxKey",    mapMaxKey_);
    registerFunction(vm, "mapKeys",      mapKeys_);
    registerFunction(vm, "mapValues",    mapValues_);
    registerFunction(vm, "mapToArray",   mapToArray_);
    registerAlias(vm, "有序映射",        "mapNew");
    registerAlias(vm, "有序插入",        "mapInsert");
    registerAlias(vm, "有序查找",        "mapFind");
    registerAlias(vm, "有序包含",        "mapContains");
    registerAlias(vm, "有序删除",        "mapErase");
    registerAlias(vm, "有序大小",        "mapSize");
    registerAlias(vm, "有序为空",        "mapIsEmpty");
    registerAlias(vm, "有序清空",        "mapClear");
    registerAlias(vm, "有序下界",        "mapLowerBound");
    registerAlias(vm, "有序上界",        "mapUpperBound");
    registerAlias(vm, "有序最小键",      "mapMinKey");
    registerAlias(vm, "有序最大键",      "mapMaxKey");
    registerAlias(vm, "有序键列表",      "mapKeys");
    registerAlias(vm, "有序值列表",      "mapValues");
    registerAlias(vm, "有序转数组",      "mapToArray");
}

} // namespace cplang
